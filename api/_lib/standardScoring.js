...
function calculateStandardFareScore({ distanceKm = 0, totalFuel = 0, condition = 'ANY_SWAP', finalStars = 3, eventPoints = 0 }) {
  ...
  const finalScore = Math.max(0, Math.round(objectiveFareValue * passengerMultiplier + boundedEventPoints));
  return { objectiveFareValue, passengerMultiplier, eventPoints: boundedEventPoints, finalScore };
}
